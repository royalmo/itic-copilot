...
(function () {
    ...
    for (i = 0; i<links.length; i++) {
        ...
        if (links[i].href.includes("@@download")) {
            links[i].parentElement.innerHTML = links[i].parentElement.innerHTML + '<a class="copilot_file_view" href="' + links[i].href.replace("@@download", "@@display-file") + '" style="display:none;color:grey;font-size:10px">[<em>' + t('ocw_replacer_view') + '</em>]</a>';
        }
        ...
        if (links[i].classList.contains('navTreeCurrentItem') && links[i].classList.contains('contenttype-folder')) {
            document.getElementById('viewlet-below-content-title').innerHTML += '<p><a class="copilot_download_folder" style="display:none;color:#007bb1;font-size:13px;" href="#"> [<em>' + t('ocw_replacer_download_all') + '</em>]</a></p><br/>';
        }
    }
    ...
    for (i = 0; i<spans.length; i++) {
        ...
        if (spans[i].children[0].classList.contains('contenttype-file')) {
            spans[i].innerHTML = spans[i].innerHTML + '<a class="copilot_file_view" href = "' + spans[i].children[0].href.replace('/view', '') + '" style="display:none;color:grey;font-size:10px">[<em>' + t('ocw_replacer_view') + '</em>]</a>';
        }

        // Download entire subject (on menu)
        if (spans[i].children[0].classList.contains('contenttype-assignatura')) {
            spans[i].innerHTML = spans[i].innerHTML + '<a class="copilot_download_subject" style="display:none; color:#2F4F4F; font-size:10px;" href="#"> [<em>' + t('ocw_replacer_download_all') + '</em>]</a>';
        }
    }
    ...
    for (i = 0; i<subjects.length; i++) {
        ...
        subjects[i].innerHTML = subjects[i].innerHTML + '<a class="copilot_download_subject" style="display:none;color:#2F4F4F; font-size:10px;" href="#"> [<em>' + t('ocw_replacer_download_all') + '</em>]</a>';
    }
    ...
    for (i = 0; i<quadrimesters.length; i++) {
        ...
        quadrimesters[i].innerHTML = quadrimesters[i].innerHTML + '<a class="copilot_download_quadrimester" style="display:none;color:#bfbbb2;font-size:10px;" href="#"> [<em>' + t('ocw_replacer_download_all') + '</em>]</a>';
    }
